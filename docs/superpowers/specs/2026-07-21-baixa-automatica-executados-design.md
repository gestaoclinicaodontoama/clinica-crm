...
5. ⚠️ **Seeding do ledger:** o pré-consumo do item 2 é semeado do estado PRÉ-BAIXA do banco, UMA vez por (paciente, dia), no primeiro toque; depois disso APENAS o ledger em memória decide consumo — nunca re-consultar o banco por item (as próprias retroativas recém-gravadas, agora com tempo NOT NULL, seriam lidas como manual e re-disparariam o pré-consumo). O ledger vive no escopo do syncPlanejamento (atravessa planos — 2 planos do mesmo paciente no dia não duplicam).
...
7. `planos` do select do syncPlanejamento ganha `paciente_clinicorp_id` (hoje não vem).
8. **Skew por-etapa documentado:** uma etapa carrega a sessão e as irmãs ficam 0 — o TOTAL do plano/dia bate com o chair time real (o que a fiscalização ③ consome); visão por-etapa individual não é métrica.
