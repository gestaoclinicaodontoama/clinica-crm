...
1. **Tratamentos executados hoje:** `/estimates/list` com `from=to=hoje`, filtrando
   itens `Executed` com data de execução hoje. (1 chamada; `estimates/list` aceita
   intervalo ≤31 dias, um dia só está ok.)
2. **Avaliações de hoje (Marcos/Matheus):** `/appointment/list` com `from=to=hoje`,
   filtrando `Dentist_PersonId`/`ScheduleToId` em `DENTISTAS_AVALIACAO`
   (`server.js:3605` — Marcos `5757301300985856`, Matheus G. `6576596377468928`) e
   **comparecimento** pela mesma lógica do sync de comparecimento existente
   (status de check-in "2-Em espera" ou posterior). ⚠️ Validar na 1ª semana se a
   recepção mantém o status da agenda atualizado — aceito pelo Luiz.
3. Total ≤ ~4 chamadas Clinicorp, dentro do limite de 25/h.
