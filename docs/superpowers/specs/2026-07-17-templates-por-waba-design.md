...
1. **Coletar as WABAs candidatas** *(mecanismo validado em produção 17/07/26 — os tokens reais, de usuário de sistema, não respondem a `me/whatsapp_business_accounts` nem trazem `target_ids` no `debug_token`)*: partir da WABA do env (`WA_BUSINESS_ACCOUNT_ID`, a do 8700) → `?fields=owner_business_info` dá o negócio dono (606620656374889) → `/{business}/owned_whatsapp_business_accounts` lista todas (4 no caso: 8700, 2873, conta de teste, GUSHUP legada). `me/whatsapp_business_accounts` fica como fallback para outros formatos de token. Lembrar qual token enxergou cada WABA e usá-lo nas chamadas dela.
2. **Filtrar pelas WABAs que possuem um dos números CONFIGURADOS** (`defaultPhoneId`/`broadcastPhoneId`), consultando `/{waba-id}/phone_numbers` e casando os ids — descarta a conta de teste e a legada GUSHUP (que tem inclusive um "8700" duplicado com outro phone_number_id). O template é associado ao número configurado encontrado. Depois buscar `message_templates` (com paginação, como já é hoje).
