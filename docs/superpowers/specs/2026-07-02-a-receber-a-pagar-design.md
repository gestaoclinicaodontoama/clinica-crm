...
- ⚠️ **A API trunca cada chamada em ~12 meses** (POR CHAMADA, não a partir de hoje — correção do Luiz em 2026-07-02: janela de 25 meses numa chamada só retornou 12 itens, mas pedir jul/27→jul/28 separado retorna os 12 seguintes). O sync fatia a janela de 25 meses em 3 chamadas (`janelasDe12m`: 12+12+1, com 400ms entre elas) e cobre os 24m inteiros dos DOIS lados. Nos meses distantes o out_forecast mostra só recorrentes provisionadas (~R$32k/mês) — a nota da página avisa que a saída real tende a ser maior.
