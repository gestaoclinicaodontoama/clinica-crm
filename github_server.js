...
app.patch('/api/leads/:id', patchLead);

// Backward-compat: rota /status antiga
app.patch('/api/leads/:id/status', patchLead);
...
app.get('/api/templates/sync-meta', async (req, res) => {
  try {
    ...
    let url = `${META_TPL_API}?fields=name,status,quality_score&limit=200`;
    const allMeta = [];
    while (url) {
...
app.post('/webhooks/whatsapp', async (req, res) => {
  try {
    ...
    db.data.mensagens.push({
      ...
      texto: sanitizeStr(m.texto, 4000), wa_id: m.id, criada_em: nowLocal(),
    });
    await db.write();
    res.status(200).send('ok');
  } catch (e) {
...
async function dispararConversaoGoogle(lead) {
  ...
  await fs.promises.appendFile(csvPath, `${qf(lead.gclid)},${qf(conversionName)},${qf(time)},${lead.valor || 0},BRL\n`);
