 ...
 */
function avancarPorRegistro(statusAtual, etapasStatus) {
  if (['descartado', 'cancelado'].includes(statusAtual)) return null;
  const etapas = etapasStatus || [];
  const todasConcluidas = etapas.length > 0 && etapas.every(s => s === 'concluida' || s === 'concluida_retroativa');
...
module.exports = { transicaoValida, validarSubLotes, aplicarResync, avancarPorRegistro };
