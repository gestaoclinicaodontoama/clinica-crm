...
/** Tira o código Clinicorp do início do nome ("0110 – Consulta..." → "Consulta...") — só exibição; exige separador (não corta "3º molar"). */
function limparCodigoProc(nome) {
  return String(nome || '').replace(/^\s*\d{2,}\s*[-–—.]\s*/, '').trim();
}
...
function resumoTracker(raizes, fallbackExecutor) {
  ...
  const procedimentos = (raizes || []).map(item => {
    ...
    return { nome: limparCodigoProc(item.procedure_name), status, executor: item.profissional_executor || fallbackExecutor || null, sessoes };
  });
...
function renderTracker({ nome, concluido, resumo, proxima }) {
  ...
  return _shell(`
    ...
    ${prox}${procs}
    <footer>Dúvidas? <a href="https://wa.me/553138230061" style="color:#2f7d4f;font-weight:600">Fale com a gente no WhatsApp — (31) 3823-0061</a> 💬</footer>`);
}
...
module.exports = { primeiroNome, limparCodigoProc, resumoTracker, renderTracker, renderNeutro };
