...
const { primeiroNome, limparCodigoProc, resumoTracker, renderTracker, renderNeutro } = require('./tracker');
...
test('limparCodigoProc: tira código Clinicorp do início; preserva nomes sem código', () => {
  assert.equal(limparCodigoProc('0110 – Consulta Inicial: Exame clínico e plano de tratamento'), 'Consulta Inicial: Exame clínico e plano de tratamento');
  assert.equal(limparCodigoProc('4140 - Coroa Sobre Implante'), 'Coroa Sobre Implante');
  assert.equal(limparCodigoProc('Extração do 3º molar'), 'Extração do 3º molar');
  assert.equal(limparCodigoProc('Prevenção'), 'Prevenção');
});
test('resumoTracker: nome do procedimento sai limpo pro paciente', () => {
  const r = resumoTracker([{ procedure_name: '4140 - Coroa Sobre Implante', profissional_executor: null, plano_etapas: [], sublotes: [] }], null);
  assert.equal(r.procedimentos[0].nome, 'Coroa Sobre Implante');
});
