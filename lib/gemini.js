...
// Removes nonce-delimiter patterns from user-controlled text to neutralize prompt injection
function sanitize(text) {
  if (!text) return '';
  return text
    .replace(/<<<TRANSCRIPT_BEGIN_[0-9a-f]+>>>/g, '')
    .replace(/<<<TRANSCRIPT_END_[0-9a-f]+>>>/g, '');
}
...
async function detalharEtapa({ etapaIdx, etapaNome, trechos, nota, dentistId, supabase }) {
  ...
  const trechosText = Array.isArray(trechos) ? trechos.join('\n') : String(trechos || '');

  const prompt = template
    .replace('{ETAPA}', etapaNome)
    .replace('{NOTA}', String(nota))
    .replace(/\{NONCE\}/g, n)
...
async function gerarInsights({ feedbacks }) {
  ...
  const feedbacksText = Array.isArray(feedbacks)
    ? feedbacks.map((f, i) => `[${i + 1}] ${JSON.stringify(f)}`).join('\n')
    : String(feedbacks || '');

  const prompt = template.replace('{FEEDBACKS}', feedbacksText);
