// módulo de browser (public/js/janela24h.js): teste vive em lib/ porque o glob do npm test é lib/**
...
test('recebida ha 2h = aberta com ~22h restantes', () => {
  ...
  assert.match(j.label, /fecha em 22h/);
});
...
test('data inválida = fechada (fail-closed, nunca aberta com NaN)', () => {
  const j = estadoJanela('data-invalida', Date.now());
  assert.strictEqual(j.estado, 'fechada');
  assert.strictEqual(j.restanteMs, 0);
});
