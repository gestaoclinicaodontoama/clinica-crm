...
async function _loop(campanhaId, deps) {
  ...
  while (true) {
    ...
    try {
      // Se o contato já vem com lead_id (ex.: disparo a partir de um Público, onde
      // os leads já existem), respeita-o — re-resolver por telefone poderia casar o
      // lead errado em famílias que compartilham os últimos dígitos. Sem lead_id
      // (fluxo CSV), resolve/cria normalmente.
      const lead_id = contato.lead_id
        || (await resolverLead(supabase, contato, camp.nome, criarLeadLeve)).lead_id;
