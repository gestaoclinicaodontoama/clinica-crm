...
async function _loginAgenteCampanha(agentToken) {
  ...
  // Preferir a campanha MANUAL dedicada ao click-to-call (nome contém "click").
  // A lista da API de agente só traz {id,name,paused} — sem type — então o nome é o
  // único sinal disponível. Evita cair numa preditiva (onde o agente não fica ocioso
  // e o manual_call/enter falha com "Agente não está ocioso"). Confirmado em prod
  // 2026-06-26: login na "CRM - Click to Call" + enter retorna 204.
  const campanha =
    lista.find(c => /click/i.test(c.name || '')) ||
    lista.find(c => /manual/i.test(c.type || c.campaign_type || c.mode || c.name || '')) ||
...
async function ligar({ agentToken, numeroDestino }) {
  ...
  if (!ok(r1)) {
    ...
    // 422 "não ocioso" = agente em estado travado (modo manual de uma ligação
    // anterior) ou não logado numa campanha. Primeiro limpa um possível modo
    // manual preso (causa o "ramal ocupado" logo após uma ligação), depois loga.
    if (r1.status === 422) {
      console.log('[3cplus] enter 422, limpando estado travado e tentando login...');
      await apiRequest('POST', '/api/v1/agent/manual_call/exit', null, agentToken).catch(() => {});
...
// Login antecipado: loga o agente na campanha manual assim que o softfone conecta,
// para que ele já esteja "ocioso/pronto" e a 1ª ligação não tenha o atraso do
// login-na-hora-de-discar. Idempotente: logar de novo já logado é inofensivo.
async function loginManual(agentToken) {
  if (!agentToken) return false;
  return _loginAgenteCampanha(agentToken).catch(() => false);
}

module.exports = { ligar, getCalls, downloadGravacao, temToken, apiRequest, loginManual };
