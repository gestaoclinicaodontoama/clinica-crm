...
async function ligar({ agentToken, numeroDestino }) {
  ...
  // Passo 1: entrar em modo manual
  const r1 = await apiRequest('POST', '/agent/manual_call_enter', null, agentToken);
  ...
  const r2 = await apiRequest('POST', '/agent/manual_call_dial', { phone: destino }, agentToken);
...
async function getCalls({ startDate, endDate } = {}) {
  ...
  const path = '/calls' + (params.toString() ? '?' + params.toString() : '');
  return apiRequest('GET', path, null, THREEC_TOKEN());
}
...
module.exports = { ligar, getCalls, downloadGravacao, temToken };
