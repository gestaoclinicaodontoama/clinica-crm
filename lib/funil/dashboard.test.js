...
test('monta payload com funil, kpis, comparacao, serie e periodo', async () => {
  const deps = {
    buscarCoorte: async (_sb, from) => fakeCoorte(from.includes('2026-04') ? 'anterior' : 'atual'),
    buscarAtividade: async () => [
      { tipo: 'historico_lead_criado', criado_em: '2026-05-10T10:00:00-03:00' },
      { tipo: 'historico_fechou', criado_em: '2026-05-10T11:00:00-03:00' },
    ],
  };
  const periodo = {
  ...
  assert.strictEqual(out.serie.pontos.length, 1);        // série vem da atividade, não da coorte
  assert.strictEqual(out.serie.pontos[0].fechamentos, 1);
