...
function normalizarEventos(eventos, leadValor = new Map()) {
  ...
  for (const [leadId, evs] of porLead) {
    ...
    let vendaAntigo = 0, fechouAntigo = false, fechouNovo = false;
    ...
    for (const e of evs) {
      ...
      if (etapa === 'fechou') {
        if (String(e.tipo).startsWith('historico_')) { fechouAntigo = true; vendaAntigo += Number(e.metadata?.valor || 0); }
        else fechouNovo = true;
      }
    }
    ...
    // regra 4: Venda de UMA fonte — planilha (era antiga) tem precedência; senão leads.valor
    if (fechouAntigo) vendaPorLead.set(leadId, vendaAntigo);
    else if (fechouNovo) vendaPorLead.set(leadId, Number(leadValor.get(leadId) || 0));
  }
