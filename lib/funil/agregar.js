...
function computeView(avaliacoes, orcamentos, base) {
  ...
  // Denominador de comparecimento: visão leads ancorada em leads_agendados; clínica em agendamentos.
  const denComparecimento = base ? leads_agendados : agendamentos;

  return {
    leads_criados,
    agendamentos,
    // % de agendamentos só faz sentido na visão leads (sobre leads criados)
    pct_agendamentos:     base ? pct(agendamentos, leads_criados) : null,
    leads_agendados,
