...
describe('resolvePeriodo', () => {
  ...
  it('preset hoje = só o dia atual, anterior = ontem', () => {
    const p = resolvePeriodo('hoje', null, null, now);
    assert.strictEqual(p.from, '2026-06-15T00:00:00-03:00');
    assert.strictEqual(p.to, '2026-06-15T23:59:59-03:00');
    assert.strictEqual(p.anterior.from, '2026-06-14T00:00:00-03:00');
    assert.strictEqual(p.anterior.to, '2026-06-14T23:59:59-03:00');
  });

  it('preset 7d = 7 dias incluindo hoje, anterior = 7 dias antes', () => {
    const p = resolvePeriodo('7d', null, null, now);
    assert.strictEqual(p.from, '2026-06-09T00:00:00-03:00');
    assert.strictEqual(p.to, '2026-06-15T23:59:59-03:00');
    assert.strictEqual(p.anterior.from, '2026-06-02T00:00:00-03:00');
    assert.strictEqual(p.anterior.to, '2026-06-08T23:59:59-03:00');
  });
});
