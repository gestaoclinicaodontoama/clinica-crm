...
async function montarDashboard(sb, periodo, origem, deps = {}) {
  // unificado=true → lê as 2 eras (CRM Antigo + Novo) via normalizar (Etapa 3). Default: só CRM Antigo (Etapa 1).
  const unificado = deps.unificado;
  const buscarCoorte = deps.buscarCoorte || (unificado ? eventosMod.buscarCoorteUnificado : eventosMod.buscarCoorte);
  const buscarAtividade = deps.buscarAtividade || eventosMod.buscarAtividade;
  const montar = (c) => unificado
    ? eventosMod.montarCoorteUnificado(c.criados, c.eventos, c.origemPorLead, c.leadValor, origem)
    : eventosMod.montarCoorte(c.criados, c.eventos, c.origemPorLead, origem);
  ...
  const cAtual = montar(atual);
  const cAnterior = montar(anterior);
