...
const { montarCoorte, montarCoorteUnificado, buscarTodos, withTimeout } = require('./eventos');
...
test('montarCoorteUnificado: une as 2 eras; venda/entrada de cada fonte (Etapa 3)', () => {
  const criados = [
    { lead_id: 1, criado_em: '2026-03-01T10:00:00-03:00' }, // CRM Antigo
    { lead_id: 2, criado_em: '2026-07-01T10:00:00-03:00' }, // CRM Novo
  ];
  const eventos = [
    { lead_id: 1, tipo: 'historico_lead_criado', criado_em: '2026-03-01T10:00:00-03:00', metadata: { origem: 'Meta Ads' } },
    { lead_id: 1, tipo: 'historico_agendado', criado_em: '2026-03-02T10:00:00-03:00' },
    { lead_id: 1, tipo: 'historico_fechou', criado_em: '2026-03-10T10:00:00-03:00', metadata: { valor: 20000, entrada: 5000 } },
    { lead_id: 2, tipo: 'lead_criado', criado_em: '2026-07-01T10:00:00-03:00', metadata: { origem: 'Indicação' } },
    { lead_id: 2, tipo: 'status_mudou', criado_em: '2026-07-03T10:00:00-03:00', metadata: { para: 'Orçado' } },
    { lead_id: 2, tipo: 'status_mudou', criado_em: '2026-07-05T10:00:00-03:00', metadata: { para: 'Fechou' } },
  ];
  const origemPorLead = new Map([[1, 'Meta Ads'], [2, 'Indicação']]);
  const leadValor = new Map([[2, 8000]]);
  const r = montarCoorteUnificado(criados, eventos, origemPorLead, leadValor, null);
  const n = Object.fromEntries(r.etapas.map(e => [e.id, e.n]));
  assert.strictEqual(n.leads, 2);
  assert.strictEqual(n.agendados, 1);   // lead 1 (antigo)
  assert.strictEqual(n.orcaram, 1);     // lead 2 (Orçado, novo)
  assert.strictEqual(n.fecharam, 2);    // ambos
  assert.strictEqual(r.kpis.venda, 28000);  // 20000 planilha + 8000 leads.valor
  assert.strictEqual(r.kpis.entrada, 5000); // só o antigo tem entrada

  // filtro de origem isola a era certa
  const r2 = montarCoorteUnificado(criados, eventos, origemPorLead, leadValor, 'Indicação');
  assert.strictEqual(r2.kpis.leads, 1);
  assert.strictEqual(r2.kpis.fechamentos, 1);
  assert.strictEqual(r2.kpis.venda, 8000);
});
