...
const LIMITES = { tipicoMin: 3, fracaoAbaixo: 0.40, travadoMin: 120, travadoMaxMin: 360, historico: 60 };
...
function estadoJob(rowsJob, jobDef, agora) {
  ...
  // "travou" só para trava RECENTE: linha mais recente incompleta, começada entre
  // travadoMin e travadoMaxMin atrás. Mais velha = abandonada (container reiniciou);
  // ignora, senão uma rodada manual morta marca o sync como quebrado pra sempre.
  if (ultima && !ultima.finished_at) {
    const idadeMin = (agora.getTime() - new Date(ultima.started_at).getTime()) / 60000;
    if (idadeMin > LIMITES.travadoMin && idadeMin <= LIMITES.travadoMaxMin) {
  ...
  if (agora >= limite) {
    // "rodou hoje?" exige rodada COMPLETA na janela — uma que começou e travou não conta.
    const rodouHoje = (rowsJob || []).some(r => completa(r) && new Date(r.started_at) >= janela);
...
// ── Fase 2: heartbeat dos jobs órfãos (job_health) ──────────────────────────
...
module.exports = {
  ...
  classificarJob, avaliarGatilhosJobs,
};
