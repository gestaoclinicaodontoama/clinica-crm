...
test('montarFatos: distribuição de lucro (grupo 8) fora do resultado/margem, reportada à parte', () => {
  const m = mes('2026-06', 100000, 40000, 20000);
  m.grupos.push({ codigo: '8', total: -30000, contas: [{ codigo: '8.1', nome: 'Distribuição de lucro — sócios', total: -30000 }] });
  m.resultado -= 30000;
...
test('topDesvios: ignora o grupo 8', () => {
  const m6 = (ym, v) => {
    const m = mes(ym, 100000, 40000, 20000);
    m.grupos.push({ codigo: '8', total: -v, contas: [{ codigo: '8.1', nome: 'Distribuição de lucro — sócios', total: -v }] });
    return m;
  };
  const d = topDesvios([m6('2026-06', 90000)], [m6('2026-04', 1000), m6('2026-05', 1000)], 3);
  assert.ok(!d.find(x => x.codigo === '8.1'));
});
