...
test('montarFatos: distribuição de lucro (grupo 6) fora do resultado/margem, reportada à parte', () => {
  const m = mes('2026-06', 100000, 40000, 20000);
  m.grupos.push({ codigo: '6', total: -30000, contas: [{ codigo: '6.1', nome: 'Distribuição de lucro — sócios', total: -30000 }] });
  m.resultado -= 30000;
  const f = montarFatos({ periodo: [m] });
  assert.equal(f.resultado, 40000);        // antes da distribuição
  assert.equal(f.margem, 0.4);
  assert.equal(f.distribuicoes, 30000);
});

test('topDesvios: ignora o grupo 6', () => {
  const m6 = (ym, v) => {
    const m = mes(ym, 100000, 40000, 20000);
    m.grupos.push({ codigo: '6', total: -v, contas: [{ codigo: '6.1', nome: 'Distribuição de lucro — sócios', total: -v }] });
    return m;
  };
  const d = topDesvios([m6('2026-06', 90000)], [m6('2026-04', 1000), m6('2026-05', 1000)], 3);
  assert.ok(!d.find(x => x.codigo === '6.1'));
});
