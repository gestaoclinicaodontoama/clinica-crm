...
// A API trunca cada chamada em ~12 meses (por CHAMADA, não a partir de hoje) —
// fatia a janela de 25 meses em sub-janelas de até 12 p/ cobrir os 24m inteiros.
function janelasDe12m(hojeISO) {
  const out = [];
  let [y, m] = hojeISO.slice(0, 7).split('-').map(Number);
  let from = hojeISO;
  for (let i = 0; i < 25; i += 12) {
    const nMeses = Math.min(12, 25 - i);
    const fim = new Date(Date.UTC(y, m - 1 + nMeses, 0)).toISOString().slice(0, 10);
    out.push({ from, to: fim });
    m += nMeses; while (m > 12) { m -= 12; y++; }
...
module.exports = { parseCashFlow, janela24m, janelasDe12m, totais, agruparParcelasPorMes };
