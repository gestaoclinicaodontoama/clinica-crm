...
// Triângulo de safra: fração ACUMULADA do valor aprovado recebida por idade
// (meses entre o mês da safra e o mês do recebimento). Censura à direita
// explícita: o array para na idade atual da safra — célula futura não existe.
function curvaSafra(items, hojeISO, nSafras = 12, maxIdade = 12) {
  const ordem = mesesAtras(hojeISO, nSafras);
  const idx = new Map(ordem.map((k, i) => [k, i]));
  const mesAtual = hojeISO.slice(0, 7);
  const mDiff = (a, b) => { const [y1, m1] = a.split('-').map(Number);
    const [y2, m2] = b.split('-').map(Number); return (y2 - y1) * 12 + (m2 - m1); };
  const acc = ordem.map(() => ({ aprovado: 0, porIdade: new Array(maxIdade).fill(0) }));
  for (const it of (items || [])) {
    if (cancelada(it)) continue;
    const k = ym(it.PostDate);
    if (!idx.has(k)) continue;
    const s = acc[idx.get(k)];
    const v = valor(it);
    s.aprovado += v;
    if (recebida(it) && dia(it.ReceivedDate) && dia(it.ReceivedDate) !== '0001-01-01') {
      const idade = Math.min(maxIdade - 1, Math.max(0, mDiff(k, ym(it.ReceivedDate))));
      s.porIdade[idade] += v;
    }
  }
  return ordem.map((k, i) => {
    const s = acc[i];
    const idadeAtual = Math.min(maxIdade - 1, mDiff(k, mesAtual));
    const curva = [];
    let cum = 0;
    for (let a = 0; a <= idadeAtual; a++) {
      cum += s.porIdade[a];
      curva.push(s.aprovado ? Math.round(cum / s.aprovado * 1000) / 1000 : 0);
    }
    return { safra: k, curva };
  });
}

module.exports = { decomposicao12m, taxaRealizacao, realizacaoPorMes, mesCorrente,
  colchao, rumoAoDegrau, razaoEntradaVenda, calculadoraMes, diasUteisRestantes, safras, curvaSafra,
