...
test('tem 33 contas com codigo unico', () => {
  assert.equal(CONTAS.length, 33);
  const cods = new Set(CONTAS.map(c => c.codigo));
  assert.equal(cods.size, 33);
});

test('pró-labore sócios (3.2.3) existe no grupo 3.2', () => {
  const c = byCodigo('3.2.3');
  assert.equal(c.nome, 'Pró-labore sócios — fixo');
...
test('cascata DRE na ordem certa (8 por último: abaixo do Resultado Final)', () => {
  assert.deepEqual(GRUPOS_DRE.map(g => g.codigo), ['1','2','3.0','3.1','3.2','3.3','4','5','7','8']);
});
