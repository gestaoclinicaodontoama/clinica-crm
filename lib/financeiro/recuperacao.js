...
function pontoDesistencia(items, hojeISO) {
  ...
  for (const it of (items || [])) {
    ...
    const pos = byT.get(t);
    let cur = pos.get(n);
    if (!cur) { cur = { paga: false, vencidaAberta: false }; pos.set(n, cur); }
    const due = dia(it.DueDate);
  ...
  let totalTravados = 0;
  for (const posMap of byT.values()) {
    const nums = [...posMap.keys()];
    if (!nums.length) continue;
    const planLen = Math.max(...nums) + 1;
    const pagas = nums.filter(k => posMap.get(k).paga);
    const ultimaPaga = pagas.length ? Math.max(...pagas) : -1;
    const travou = nums.some(k => k > ultimaPaga && posMap.get(k).vencidaAberta && !posMap.get(k).paga);
    if (!travou) continue;
    totalTravados++;
  ...
  // moda determinística: mais planos; empate → menor parcela (parouEm já ordenado asc)
  let modaParouEm = null, modaN = 0;
  for (const { parcela, planos } of parouEm) if (planos > modaN) { modaN = planos; modaParouEm = parcela; }
