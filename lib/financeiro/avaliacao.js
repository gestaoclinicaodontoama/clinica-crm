...
// fixo×variável com a exceção 3.2.3 (pró-labore sócios = fixa) — mesma régua da página
const somaVariaveis = (meses) => meses.reduce((s, m) => s + A.variaveisDe(m), 0);
const somaFixas = (meses) => meses.reduce((s, m) => s + A.fixasDe(m), 0);
...
function topDesvios(periodo, contexto, n = 3) {
  const acumular = (meses) => {
    ...
    for (const m of meses) for (const g of (m.grupos || [])) {
      if (g.codigo === '1' || g.codigo === '8' || g.codigo === '9') continue; // 8 = distribuição, 9 = provisão: fora do estouro de custo
...
function montarFatos({ periodo, contexto = [], anoAnterior = [] }) {
  ...
  const fatos = {
    ...
    variaveisPctReceita: receita > 0 ? pct(Math.abs(somaVariaveis(periodo)) / receita) : null,
    fixasMediaMes: arred(Math.abs(somaFixas(periodo)) / periodo.length),
  };
  ...
  if (contexto.length) {
    ...
    fatos.contexto = {
      ...
      variaveisPctReceita: recCtx > 0 ? pct(Math.abs(somaVariaveis(contexto)) / recCtx) : null,
      fixasMediaMes: arred(Math.abs(somaFixas(contexto)) / contexto.length),
    };
