...
function agregarPorPaciente(items, today) {
  ...
  for (const it of (items || [])) {
    ...
    if (!m[id]) m[id] = {
      ...
      parcelasVencidas: [], parcelasFuturas: [],
    };
    const p = m[id];
    ...
    if (due < today) {
      ...
      p.parcelasVencidas.push({ due, valor: v, forma: String(it.PaymentForm || '') });
      ...
      p.parcelasFuturas.push({ due, valor: v, forma: String(it.PaymentForm || '') });
    }
  }
  ...
  return Object.values(m).filter(p => p.overdueCount > 0).map(p => {
    ...
    // Ficha de auditoria: as listas são os COMPONENTES exatos dos totais acima.
    const porDue = (a, b) => (a.due < b.due ? -1 : 1);
    p.parcelasVencidas = p.parcelasVencidas.sort(porDue)
      .map(x => ({ ...x, valor: Math.round(x.valor * 100) / 100 }));
    p.parcelasFuturas = p.parcelasFuturas.sort(porDue)
      .map(x => ({ ...x, valor: Math.round(x.valor * 100) / 100 }));
    return p;
  });
}
