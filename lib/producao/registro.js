...
function classificarDia({ atendimentos, producao, pacientesComPlanoAtivo }) {
  ...
  const comPlanoAtivo = pacientesComPlanoAtivo || new Set();
  ...
  for (const a of (atendimentos || [])) {
    ...
    // Modo de Planejamento: sessão sem registro de paciente com plano de
    // tratamento ativo é "esperada pelo plano" — intermediária, não pendência.
    // Fora da contagem de atendidos/pendentes, igual à Manutenção.
    if (!item.registrado && pid && comPlanoAtivo.has(pid)) { esperada_plano.push(item); continue; }
