...
test('paciente com registro por sessão hoje vira esperada_plano e sai de pendentes', () => {
  const atendimentos = [
    at({ paciente_clinicorp_id: 'P1', patient_name: 'Maria', dentist_name: 'Dr. M' }),
    at({ paciente_clinicorp_id: 'P2', patient_name: 'João',  dentist_name: 'Dr. M' }),
  ];
  const r = classificarDia({ atendimentos, producao: [], pacientesRegistradosHoje: new Set(['P1']) });
  assert.strictEqual(r.esperada_plano.length, 1);
  assert.strictEqual(r.esperada_plano[0].paciente_clinicorp_id, 'P1');
  assert.ok(!r.sem_registro.some(a => a.paciente_clinicorp_id === 'P1'));
  assert.strictEqual(r.resumo.pendentes, 1);            // só o João conta como pendência
});

test('esperada_plano fica fora de por_dentista (igual Manutenção)', () => {
  const atendimentos = [
    at({ paciente_clinicorp_id: 'P1', patient_name: 'Maria', dentist_name: 'Dr. M' }),
    at({ paciente_clinicorp_id: 'P2', patient_name: 'João',  dentist_name: 'Dr. M' }),
  ];
  const r = classificarDia({ atendimentos, producao: [], pacientesRegistradosHoje: new Set(['P1']) });
  const dm = r.resumo.por_dentista.find(d => d.dentista === 'Dr. M');
  assert.strictEqual(dm.pendentes, 1);
  assert.strictEqual(dm.atendidos, 1);
});

test('sem pacientesRegistradosHoje (default) → comportamento inalterado', () => {
  const r = classificarDia({ atendimentos: [at()], producao: [] });
  assert.strictEqual(r.esperada_plano.length, 0);
  assert.strictEqual(r.sem_registro.length, 1);
});

test('paciente com plano ativo mas SEM registro por sessão hoje → pendência real (não dispensa mais)', () => {
  const r = classificarDia({
    atendimentos: [at({ paciente_clinicorp_id: 'P1', patient_name: 'Maria', dentist_name: 'Dr. M' })],
    producao: [], pacientesRegistradosHoje: new Set(),   // plano ativo existe, mas nada registrado hoje
  });
  assert.strictEqual(r.esperada_plano.length, 0);
  assert.strictEqual(r.sem_registro.length, 1);
  assert.strictEqual(r.resumo.pendentes, 1);
