...
async function _processarUma(supabase, nota, emissor, opts = {}) {
  ...
  try {
    ...
    const persistiu = await _upd(supabase, nota.id, {
      status: 'Processando', rps_numero: rpsNumero, ambiente: _ambiente(), xml_envio: xmlEnvio,
    });
    if (!persistiu) {
      // RPS/Processando NÃO gravou — abortar ANTES de enviar. Se enviássemos mesmo assim,
      // a prefeitura receberia o RPS mas a linha ficaria Pendente sem rps_numero, e o
      // próximo lote reservaria um RPS NOVO → emissão duplicada na prefeitura.
      _log(`#${nota.id} ⚠️ não persistiu RPS/Processando — envio abortado (retenta no próximo lote)`);
      return { ok: false };
    }
    ...
    if (e instanceof NfseComunicacaoError) {
      ...
      await _upd(supabase, nota.id, { status: 'Processando', rps_numero: rpsNumero, erro_msg: `aguardando reconciliação: ${e.message}` });
...
async function processarPendentes(supabase, { onLog, chamarWsImpl } = {}) {
  ...
  try {
    ...
    _log(`sistemas com emissor ativo: ${Object.keys(emissores).join(', ')} (notas de outros sistemas não entram neste motor)`);
