...
test('falha ao gravar RPS/Processando ANTES do envio: WS NÃO é chamado (evita duplicata na prefeitura)', async () => {
  let wsChamado = false;
  const sb = {
    rpc: async () => ({ data: 10, error: null }),
    from: () => ({
      update: (patch) => ({ eq: async (c, id) => {
        if (patch.status === 'Processando') return { error: { message: 'conexão caiu' } };
        return { error: null };
      } }),
    }),
  };
  const ws = async () => { wsChamado = true; return XML_SUCESSO; };
  const r = await _processarUma(sb, nota, emissor, { chamarWsImpl: ws });
  assert.strictEqual(wsChamado, false); // envio abortado — não manda RPS pra prefeitura sem rastro no banco
  assert.strictEqual(r.ok, false);
});
